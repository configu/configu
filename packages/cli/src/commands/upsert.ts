...
${' '}
 ...
 Edit the key-value pairs below to modify Configs for the "${input.set.label}" set in the "${input.store.type}" store.
${' '}
 Annotations:
 cfgu.const  = Computed at evaluation. Cannot be edited.
 cfgu.lazy   = Assigned at evaluation. Cannot be edited.
${' '}
 ...
 Learn more: https://docs.configu.com`;
    ...
    _.chain(upsertResult.result)
      .forEach(({ key, value, cfgu }) => {
        ...
        if (cfgu?.const) {
          pairValue = null;
          comment = ` cfgu.const: ${cfgu.const}`;
        } else if (cfgu?.lazy) {
          pairValue = null;
          comment = ` cfgu.lazy: ${cfgu.lazy}`;
        }
  ...
  printUpsertReport(input: UpsertCommandInput, result: UpsertCommandOutput) {
    const data = _.chain(result)
      ...
      .map((output) => {
        ...
        if (cfgu?.const) {
          return [key, color.dim(`cfgu.const: ${cfgu.const}`)];
        }
        if (cfgu?.lazy) {
          return [key, color.dim(`cfgu.lazy: ${cfgu.lazy}`)];
        }
  ...
  async execute() {
    ...
    try {
      ...
      const hasInput = this.fromFile || this.fromLiteral || this.edit || this.prune || this.context.pipe;
      if (!hasInput && !this.context.environment.isCI) {
        this.edit = true;
      }
