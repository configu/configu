...
USAGE
  $ configu export [--explain | --format
    JSON|CompactJSON|YAML|Dotenv|KubernetesConfigMap|HelmValues|TerraformTfvars|TOML|CSV | --template <value> | --source |
...
FLAGS
  ...
  --format=<option>          Format exported `Configs` to common configuration formats. Redirect the output to file, if
                             needed
                             <options: JSON|CompactJSON|YAML|CSV|KubernetesConfigMap|HelmValues|TerraformTfvars|TOML>
...
EXAMPLES
  ...
  Pipe eval commands result to export command to create a CSV .env file

    $ configu eval ... | configu export --format 'CSV'
