...
<ResponseField name="test" type="ConfigExpression|ConfigExpression[]">
  ...
  expression that evaluates to a boolean.\ If an array is provided, all expressions must evaluate to `true` for the
  value to be considered valid. \ \ `ConfigExpression` context: - `$.value`: The `ConfigValue` to be validated. -
  `$.cfgu`: The `Cfgu` of the evaluated config. - `$.key`: The `ConfigKey` of the evaluated config. - `$.origin`: The
  origin of the `ConfigValue`. One of `empty`, `default`, `store`, `override`, or `const`. - `context.store`: The
  `ConfigStore` provided as input to the command. - `context.set`: The `ConfigSet` provided as input to the command. -
  `context.schema`: The `ConfigSchema` provided as input to the command. - All other `ConfigKey` values in the
  `ConfigSchema` are accessible in the context via their key.
</ResponseField>
...
<ResponseField name="const" type="ConfigExpression">
  ...
  `ConfigStore`.\ This property cannot be set together with `lazy`, `default`, or `required`.\ \ `ConfigExpression`
  context: - `$.cfgu`: The `Cfgu` of the evaluated config. - `$.key`: The `ConfigKey` of the evaluated config. -
  `$.origin`: The origin of the `ConfigValue`. One of `empty`, `default`, `store`, `override`, or `const`. -
  `context.store`: The `ConfigStore` provided as input to the command. - `context.set`: The `ConfigSet` provided as
  input to the command. - `context.schema`: The `ConfigSchema` provided as input to the command. TODO: verify with Ran's
  PR
</ResponseField>
...
<ResponseField name="enum" type="String[]">
  Specify a list of allowable `ConigValues` for a `ConfigKey`. \ This property cannot be set together with `template`. \
  This property cannot be an empty array or contain empty strings. \ Each option must be unique.
</ResponseField>

<ResponseField name="default" type="String">
  Set a default `ConfigValue` for a `ConfigKey` if not explicitly provided. \ This property cannot be set together with
  `required`, `const` or `lazy` properties. \ If `enum` is set, the default value must be one of the options.
</ResponseField>
