...
<ResponseField name="test" type="ConfigExpression|ConfigExpression[]">
  ...
  <br />
  If an array is provided, all expressions must evaluate to `true` for the value to be considered valid. <br />
  <br />
  `ConfigExpression` context: - `$.value`: The `ConfigValue` to be validated. - `$.cfgu`: The `Cfgu` of the evaluated
  config. - `$.key`: The `ConfigKey` of the evaluated config. - `$.origin`: The origin of the `ConfigValue`. One of
  `empty`, `default`, `store`, `override`, or `const`. - `context.store`: The `ConfigStore` provided as input to the
  command. - `context.set`: The `ConfigSet` provided as input to the command. - `context.schema`: The `ConfigSchema`
  provided as input to the command. - All other `ConfigKey` values in the `ConfigSchema` are accessible in the context
  via their key.
</ResponseField>
...
<ResponseField name="const" type="ConfigExpression">
  ...
  `ConfigStore`.
  <br />
  This property cannot be set together with `lazy`, `default`, or `required`.
  <br />
  <br />
  `ConfigExpression` context: - `$.cfgu`: The `Cfgu` of the evaluated config. - `$.key`: The `ConfigKey` of the
  evaluated config. - `$.origin`: The origin of the `ConfigValue`. One of `empty`, `default`, `store`, `override`, or
  `const`. - `context.store`: The `ConfigStore` provided as input to the command. - `context.set`: The `ConfigSet`
  provided as input to the command. - `context.schema`: The `ConfigSchema` provided as input to the command. TODO:
  verify with Ran's PR
</ResponseField>

<ResponseField name="pattern" type="String">
  Define a regular expression a `ConfigValue` must match.
  <br />
...
<ResponseField name="required" type="Boolean">
  ...
  <br />
  This property cannot be set together with `default` or `const`.
</ResponseField>
...
<ResponseField name="lazy" type="Boolean">
  ...
  assignments.
  <br />
  This property cannot be set together with `const` or `default`.
</ResponseField>
