...
The `.cfgu` file can be written in one of the following formats: `YAML` or `JSON`. The name suffix of this file should be <CfguExt /> according of the format used.
...
<ResponseField name="type" type="CfguType" required>
  ...
  <Accordion title="CfguType">
    ...
    check if the string match a regular expression.
    </ResponseField>
