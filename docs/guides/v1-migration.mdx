...
```bash
--filter expect($.labels).to.include('my-label')
```

- `--omit-label`

```bash
--filter expect($.labels).to.not.include('my-label')
```

- `--pick-key`

```bash
--filter expect($.key).to.include('my-key')
```

- `--omit-key`

```bash
--filter expect($.key).to.not.include('my-key')
```

- `--pick-hidden`

```bash
--filter expect($.cfgu.hidden).to.be.true
```

- `--omit-hidden`

```bash
--filter expect($.cfgu.hidden).to.be.false
```

- `--pick-empty`

```bash
--filter expect($.value).to.be.empty
```

- `--omit-empty`

```bash
--filter expect($.value).to.not.be.empty
```
...
V1 introduces type coercion to Configu. During evaluation, Configu will attempt to coerce values from ConfigStores to their --filter expected type. This feature is intended to make Configu more flexible and easier to use.

Type coercion affects:

- All instances of config values in `ConfigExpressions` are coerced to the --filter expected type (`string`, `number`, `boolean`, `object`, `array`). You can access the value as it's stored via `storedValue`.
...
- The Configu Proxy export route which will now coerce values to the --filter expected type.
...
- [Chai](https://github.com/chaijs/chai) for assertions functions. Use `assert`, --filter expect`, and `should`respectively within the`ConfigExpression` to access Chai functions.
...
You can now mark a `ConfigStore` as your default store when omitting `--store` flag is omitted by adding `default`=`true` to the store settings. Additionally, the CLI now detects when there is only a single `ConfigStore` in the `.configu` file and uses it as the default.

Example definition of a default store in a `.configu` file:
