...
<Info>
  - All navigation options are disabled during tag exploration. - All regular config action menu options besides `show
  value` are disabled during tag exploration. You can view the value of the configs if you have the appropriate
  permission.
</Info>
